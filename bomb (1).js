export default (() => {
  ...
  function placeBomb(p) {
    ...
    p.ownBombTile = { r, c }; // allow player to walk off the tile they just bombed
  }
  ...
  function movePlayer(p, dt) {
    ...
    function blocked(x, y) {
      return [[x-margin,y-margin],[x+margin,y-margin],[x-margin,y+margin],[x+margin,y+margin]]
        .some(([cx, cy]) => {
          const gc = Math.floor(cx / cellSize), gr = Math.floor(cy / cellSize);
          if (gr < 0 || gr >= ROWS || gc < 0 || gc >= COLS) return true;
          const t = grid[gr][gc];
          if (t === TILE.WALL || t === TILE.BLOCK) return true;
          // Allow walking through a bomb on the tile where the player just placed it
          if (t === TILE.BOMB) {
            if (p.ownBombTile && p.ownBombTile.r === gr && p.ownBombTile.c === gc) return false;
            return true;
          }
          return false;
        });
    ...
    // Try each axis independently — this fixes vertical-only movement being swallowed
    if (p.dx !== 0 && !blocked(p.x + p.dx * speed, p.y)) p.x += p.dx * speed;
    if (p.dy !== 0 && !blocked(p.x, p.y + p.dy * speed)) p.y += p.dy * speed;

    // Clear own-bomb pass-through once player has walked off that cell
    if (p.ownBombTile) {
      if (Math.floor(p.x / cellSize) !== p.ownBombTile.c ||
          Math.floor(p.y / cellSize) !== p.ownBombTile.r) p.ownBombTile = null;
    }
